...
public class UploadFileCommandHandler : IRequestHandler<UploadFileCommand, Unit>
{
    ...
    public UploadFileCommandHandler(RpcRequestSender rpc, PubSubSender pubSub, IManagerRepository manager, IBaseManagerRepository repository)
    {
        _rpc = rpc;
        _pubSub = pubSub;
        _manager = manager;
    ...
    public async Task<Unit> Handle(UploadFileCommand request, CancellationToken cancellationToken)
    {
        ...
        if (await _rpc.CheckDocumentExistence(request.UserId, DocumentType.EducationDocument, true))
            throw new BadRequest("Provided user has file");
